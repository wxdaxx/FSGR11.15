...
class ScaledDotProductAttention(nn.Module):
    ...
    def forward(self, queries, keys, values, attention_mask=None, attention_weights=None, t=None, need_attn=False):
        '''
        Computes
        ...
        if need_attn:
            if t is not None:
                np.save(f"../captions/visual/att_{t}.npy", att.cpu().numpy())
            else:
                np.save("../captions/visual/att_grids.npy", att.cpu().numpy())
...
class ScaledDotProductGeometryAttention(nn.Module):
    ...
    def forward(self, queries, keys, values, box_relation_embed_matrix, attention_mask=None, attention_weights=None, need_attn=False):
        '''
        Computes
        :param queries: Queries (b_s, nq, d_model)
        :param keys: Keys (b_s, nk, d_model)
...
class ScaledDotProductAdaptiveAttention(nn.Module):
    ...
    def forward(self, queries, keys, values, attention_mask=None, attention_weights=None, language_feature=None, drop=1):
        '''
        Computes
        :param queries: Queries (b_s, nq, d_model)
        :param keys: Keys (b_s, nk, d_model)
...
class MultiHeadAttention(Module):
    ...
    def forward(self, queries, keys, values, attention_mask=None, attention_weights=None, t=None, need_attn=False):
        ...
        if self.identity_map_reordering:
            ...
            out = self.attention(q_norm, k_norm, v_norm, attention_mask, attention_weights, t, need_attn)
            out = queries + self.dropout(torch.relu(out))
        else:
            out = self.attention(queries, keys, values, attention_mask, attention_weights, t, need_attn)
...
class MultiHeadAdaptiveAttention(Module):
    ...
    def forward(self, queries, keys, values, attention_mask=None, attention_weights=None, language_feature=None, drop=1):
        ...
        if self.identity_map_reordering:
            ...
            out = self.attention(q_norm, k_norm, v_norm, attention_mask, attention_weights, s_norm, drop)
            out = queries + self.dropout(torch.relu(out))
        else:
            out = self.attention(queries, keys, values, attention_mask, attention_weights, language_feature, drop)
            out = self.dropout(out)
            out = self.layer_norm(queries + out)
        return out
...
class MultiHeadGeometryAttention(Module):
    ...
    def forward(self, queries, keys, values, relative_geometry_weights, attention_mask=None, attention_weights=None, need_attn=False):
        if self.can_be_stateful and self._is_stateful:
            self.running_keys = torch.cat([self.running_keys, keys], 1)
            keys = self.running_keys
        ...
        if self.identity_map_reordering:
            ...
            k_norm = self.layer_norm(keys)
            v_norm = self.layer_norm(values)
            out = self.attention(q_norm, k_norm, v_norm, relative_geometry_weights, attention_mask, attention_weights, need_attn)
            out = queries + self.dropout(torch.relu(out))
        else:
            out = self.attention(queries, keys, values, relative_geometry_weights, attention_mask, attention_weights, need_attn)
            out = self.dropout(out)
            out = self.layer_norm(queries + out)
        return out
