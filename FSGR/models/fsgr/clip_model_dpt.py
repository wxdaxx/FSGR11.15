...
from models.rstnet.clip_surgery_model import CLIPSurgery
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        if self.total_d_layer == 0: #shallow
            ...
            x = self.forward_deep_prompt(x, H, W)
        elif self.total_d_layer < 0:
            x = self.forward_reverse_deep_prompt(x, H, W)
        else:
        ...
        visual_embedding = x[:, -(H*W):]

        # x = self.ln_post(x)
        # if self.proj is not None:
        #     x = x @ self.proj

        return visual_embedding, g
...
def build_model(name: str, state_dict: dict, adapter_layer_list: list):
