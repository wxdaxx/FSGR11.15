...
class ResidualAttentionBlock(nn.Module):
    def __init__(self, d_model: int, n_head: int, attn_mask: torch.Tensor = None, scale=1., num_tadapter=1, drop_path=0., adapter=True):
        super().__init__()
...
class Transformer(nn.Module):
    def __init__(self, width: int, layers: int, heads: int, attn_mask: torch.Tensor = None, scale=1., num_tadapter=1, drop_path=0.1, adapter_layer=[0, 11]):
        super().__init__()
        self.width = width
        self.layers = layers

        self.adapter_b = adapter_layer[0]
        self.adapter_e = adapter_layer[1]
        dpr = [x.item() for x in torch.linspace(0, drop_path, self.layers)]
        self.resblocks = nn.Sequential(*[ResidualAttentionBlock(width, heads, attn_mask, scale, num_tadapter, dpr[i], adapter=False) if i < self.adapter_b or i > self.adapter_e else ResidualAttentionBlock(width, heads, attn_mask, scale, num_tadapter, dpr[i], adapter=True) for i in range(layers)])
...
class VisionTransformer(nn.Module):
    def __init__(self, input_resolution: int, patch_size: int, width: int, layers: int, heads: int, output_dim: int, drop_path_rate, 
                 num_tokens=100, prompt_dim=768, total_d_layer=11, num_tadapter=1, adapter_scale=0.5, adapter_layer=[0, 11]):
        super().__init__()
        self.input_resolution = input_resolution
        ...
        self.transformer = Transformer(width, layers, heads, num_tadapter=num_tadapter, scale=adapter_scale, drop_path=drop_path_rate, adapter_layer=adapter_layer)
    ...
    def forward(self, x: torch.Tensor, return_all=False, csa=True):
        ...
        if self.total_d_layer == 0: #shallow
            ...
            x, align_feats = self.forward_deep_prompt(x, w, h)
        elif self.total_d_layer < 0:
            x, align_feats = self.forward_reverse_deep_prompt(x, w, h)
        else:
            AttributeError('Input correct total_d_layer')
        
        x = x.permute(1, 0, 2)  # LND -> NLD
        align_feats = align_feats.permute(1, 0, 2)  # LND -> NLD
        g = self.ln_post(x[:, 0, :])
        ...
        if return_all:
            return x, g, x @ self.proj

        return x

    def custom_last_layer(self, x, w, h):
        image_embeddings = x[-(w*h):, :, :]
        blk = self.transformer.resblocks[-1]
        x = blk(x)
        image_embeddings = image_embeddings + self.custom_attn(blk.attn, blk.ln_1(image_embeddings), csa=True)
        image_embeddings = image_embeddings + blk.mlp(blk.ln_2(image_embeddings))
        return x, image_embeddings

    def forward_deep_prompt(self, embedding_output, H, W, out_last=False):
        ...
        for i in range(self.num_layers):
            if i == 0:
                ...
                if i != self.num_layers - 1:
                    ...
                    hidden_states, align_feats = self.custom_last_layer(hidden_states, W, H)
            else:
                hidden_states = torch.cat((
                    hidden_states[:1, :, :],
                    hidden_states[-(H*W):, :, :]
        ...
        align_feats = self.prompt_norm(align_feats)
        if out_last:
            return before_last_feats
        else:
            return encoded, align_feats

    def forward_reverse_deep_prompt(self, embedding_output, H, W, out_last=False):
        B = embedding_output.shape[1]
        ...
        encoded = self.prompt_norm(hidden_states)
        align_feats = self.prompt_norm(hidden_states)
        if out_last:
            return before_last_feats
        else:
            return encoded, align_feats
    ...
    def __init__(self,
        ...
        if isinstance(vision_layers, (tuple, list)):
            ...
            self.visual = VisionTransformer(
                ...
                num_tadapter=1,
                adapter_scale=0.5,
                adapter_layer = adapter_layer_list
            )
...
def build_model(name, state_dict: dict, adapter_layer_list: list):
    ...
    model = CLIP(
        embed_dim,
        image_resolution, vision_layers, vision_width, vision_patch_size, adapter_layer_list,
