...
class Attention(nn.Module):
    ...
    def forward(self, x):
        B, N, C = x.shape
        qkv = self.qkv(x).reshape(B, N, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)  # (3, B, num_head, num_tokens, dim)
        q, k, v = qkv[0], qkv[1], qkv[2]

        # original self-attention for the original path (B, n_h, N, d) @ (B, n_h, d, N) -> (B, n_h, N, N)
...
class AttentionPool2d(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.attn == None:
            ...
            self.attn.qkv.weight = torch.nn.Parameter(torch.cat([self.q_proj.weight, self.k_proj.weight, self.v_proj.weight], 0))
            self.attn.qkv.bias = torch.nn.Parameter(torch.cat([self.q_proj.bias, self.k_proj.bias, self.v_proj.bias]))
    ...
    def forward(self, x):
        ...
        if isinstance(self.attn, Attention):
            if isinstance(x, list):
                ...
                if self.adapter:
                    x_res = self.S_Adapter(x_res)
                    x_ori_res = self.S_Adapter(x_ori_res)
...
class VisionTransformer(nn.Module):
    def __init__(self, input_resolution: int, patch_size: int, width: int, layers: int, heads: int, output_dim: int, drop_path_rate, num_tadapter=1, adapter_scale=0.5, adapter_layer=[0, 11]):
        super().__init__()
        self.input_resolution = input_resolution
        self.output_dim = output_dim
